...
/mob/living/simple_animal/hostile/megafauna/firegecko
	name = "Kuna Bogwezhi"
	desc = "A monstrous fire gekko which towers over its' prey. It is incredibly dangerous, slaughtering numerous hunters who would try to fashion it's hide."
	health = 2500
	maxHealth = 2500
	spacewalk = TRUE
	...
	deathmessage = "collapses onto the floor, its' impact shaking the ground."
...
/mob/living/simple_animal/hostile/megafauna/firegecko/Initialize()
	smallsprite.Grant(src)
	. = ..()

/mob/living/simple_animal/hostile/megafauna/firegecko/BiologicalLife(seconds, times_fired)
	if(!(. = ..()))
		return
	move_to_delay = clamp(round((health/maxHealth) * 10), 3, 6)

/mob/living/simple_animal/hostile/megafauna/firegecko/ex_act(severity, target)
	if(severity == 3)
		return
	..()

/mob/living/simple_animal/hostile/megafauna/firegecko/adjustHealth(amount, updating_health = TRUE, forced = FALSE)
	if(!forced && (swooping & SWOOP_INVULNERABLE))
		return FALSE
	return ..()

/mob/living/simple_animal/hostile/megafauna/firegecko/visible_message(message, self_message, blind_message, vision_distance = DEFAULT_MESSAGE_RANGE, list/ignored_mobs, mob/target, target_message, visible_message_flags = NONE)
	if(swooping & SWOOP_INVULNERABLE) //to suppress attack messages without overriding every single proc that could send a message saying we got hit
		return
	return ..()

/mob/living/simple_animal/hostile/megafauna/firegecko/AttackingTarget()
	if(!swooping)
		return ..()

/mob/living/simple_animal/hostile/megafauna/firegecko/DestroySurroundings()
	if(!swooping)
		..()

/mob/living/simple_animal/hostile/megafauna/firegecko/Move()
	if(!swooping)
		if(!stat)
			playsound(src.loc, 'sound/effects/meteorimpact.ogg', 200, 1, 2, 1)
			..()

/mob/living/simple_animal/hostile/megafauna/firegecko/Goto(target, delay, minimum_distance)
	if(!swooping)
		..()

/mob/living/simple_animal/hostile/megafauna/firegecko/OpenFire()
	if(swooping)
		return
...
/mob/living/simple_animal/hostile/megafauna/firegecko/proc/fire_rain(anger_modifier)
	if(!target)
		return
...
/mob/living/simple_animal/hostile/megafauna/firegecko/proc/fire_walls(anger_modifier)
...
/mob/living/simple_animal/hostile/megafauna/firegecko/proc/fire_wall(dir)
	...
	for(var/turf/J in getline(src,E))
		...
		for(var/mob/living/L in J.contents - hit_things)
			if(istype(L, /mob/living/simple_animal/hostile/megafauna/firegecko))
				continue
			L.adjustFireLoss(20)
			to_chat(L, "<span class='userdanger'>You're hit by the gekko's fire breath!</span>")
			hit_things += L
		previousturf = J
		sleep(1)

/mob/living/simple_animal/hostile/megafauna/firegecko/proc/shoot_projectile(turf/marker, set_angle)
...
/mob/living/simple_animal/hostile/megafauna/firegecko/proc/diagonal_shots(list/dirs)
	if(!islist(dirs))
...
/mob/living/simple_animal/hostile/megafauna/firegecko/proc/triple_swoop()
	swoop_attack(swoop_duration = 30)
	swoop_attack(swoop_duration = 20)
	swoop_attack(swoop_duration = 10)

/mob/living/simple_animal/hostile/megafauna/firegecko/proc/swoop_attack(fire_rain, atom/movable/manual_target, swoop_duration = 40)
	if(stat || swooping)
		return
	if(manual_target)
	...
	density = FALSE
	icon_state = "shadow"
	visible_message("<span class='boldwarning'>[src] jumps up high!</span>")
	...
	if(target.x < initial_x)
		...
		negative = prob(50)
	var/obj/effect/temp_visual/firegecko_flight/F = new /obj/effect/temp_visual/firegecko_flight(loc, negative)
	...
	if(negative)
		...
		if(ISINRANGE(x, initial_x - DRAKE_SWOOP_DIRECTION_CHANGE_RANGE, initial_x - 1))
			negative = TRUE
	new /obj/effect/temp_visual/firegecko_flight/end(loc, negative)
	new /obj/effect/temp_visual/firegecko_swoop(loc)
...
/mob/living/simple_animal/hostile/megafauna/firegecko/AltClickOn(atom/movable/A)
	if(!istype(A))
...
/obj/effect/temp_visual/target/proc/Gecko_fall(list/flame_hit)
	...
	for(var/mob/living/L in T.contents)
		...
		if(islist(flame_hit) && !flame_hit[L])
			L.adjustFireLoss(40)
			to_chat(L, "<span class='userdanger'>You're hit by the gecko's fire breath!</span>")
			flame_hit[L] = TRUE
		else
			L.adjustFireLoss(10) //if we've already hit them, do way less damage

/obj/effect/temp_visual/firegecko_swoop
	name = "certain death"
...
/obj/effect/temp_visual/firegecko_flight
	icon = 'icons/fallout/mobs/megafauna/64x64megafauna.dmi'
	icon_state = "mega_gekko"
...
/obj/effect/temp_visual/firegecko_flight/end
	pixel_x = DRAKE_SWOOP_HEIGHT
	pixel_z = DRAKE_SWOOP_HEIGHT
	duration = 5

/obj/effect/temp_visual/firegecko_flight/end/flight(negative)
	if(negative)
		pixel_x = -DRAKE_SWOOP_HEIGHT
